...
const Trade = () => {
    ...
    useEffect(() => {
        ...
        if (location && location.pathname && chainId) {
            ...
            let token
            if (chainId === 1337) {
                token = TOKENS.LOCAL.find(item => item.address.toLowerCase() === contractAddress.toLowerCase())
            } else if (chainId === 42) {
                token = TOKENS.KOVAN.find(item => item.address.toLowerCase() === contractAddress.toLowerCase())
            }
            setToken(token)
        }
