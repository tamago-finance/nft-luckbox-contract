...
contract PMM is Lockable, Whitelist, IPmm {
    ...
    // Position Manager contract address (for tracking only)
    ...
    constructor(
        ...
        require(address(_positionManagerAddress) != address(0), "Invalid perpertual addresss");

        priceFeeder = IPriceFeeder(_priceFeeder);
        baseToken = IERC20(_baseToken);
        quoteToken = IERC20(_quoteToken);
        positionManager = _positionManagerAddress;
        ...
        addAddress(_positionManagerAddress);
        emit CreatedPMM();
    ...
    // // INTERNAL FUCTIONS
    function _validateParameters() internal view returns (uint256) {
        require(k < ONE, "K>=1");
        require(k > 0, "K=0");
    }
