  ...
  Pausable
{
  ...
  function cancelCrosschainOrder(uint256 _orderId, address _to)
    external
    onlyAdmin
    nonReentrant
  {
  ...
  function eligibleToSwap(
    ...
    return _eligibleToSwap(_orderId, _assetAddress, _tokenIdOrAmount, _proof);
  }
  ...
  modifier validateId(uint256 _orderId) {
    require(orders[_orderId].active == true, "Given ID is invalid");
    require(orders[_orderId].canceled == false, "The order has been cancelled");
    require(orders[_orderId].ended == false, "The order has been fulfilled");
    _;
  }
  ...
  modifier validateIds(uint256[] memory _orderIds) {
    ...
    for (uint256 i = 0; i < _orderIds.length; i++) {
      ...
      require(orders[i].canceled == false, "The order has been cancelled");
  ...
  function _eligibleToSwap(
    ...
    bytes32 leaf = keccak256(abi.encodePacked(_assetAddress, _tokenIdOrAmount));
